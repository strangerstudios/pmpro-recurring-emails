	...
	foreach ( $emails as $days => $template ) {
		...
		foreach ( $subscriptions_to_notify as $subscription_to_notify ) {
			...
			if ( ! empty( $recurring_notice_meta ) ) {
				// Get the lowest $days for the old meta keys. We'll assume that this reminder period has already been sent.
				$lowest_days = null;

				// Delete the old user meta and set the new subscription meta.
				foreach ( $recurring_notice_meta as $meta ) {
					$meta_days = str_replace( 'pmpro_recurring_notice_', '', $meta->meta_key );
					if ( is_null( $lowest_days ) || $meta_days < $lowest_days ) {
						$lowest_days = $meta_days;
					}
				...
				if ( $lowest_days <= $days ) {
					update_pmpro_subscription_meta( $subscription_to_notify->id, 'pmprorm_last_next_payment_date', $subscription_to_notify->next_payment_date );
					update_pmpro_subscription_meta( $subscription_to_notify->id, 'pmprorm_last_days', $lowest_days );
